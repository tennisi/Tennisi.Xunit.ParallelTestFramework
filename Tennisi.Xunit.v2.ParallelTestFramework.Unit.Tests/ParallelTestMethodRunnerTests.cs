...
{
    [TestMethod]
    public async Task ShouldNotExceedMaxConcurrency()
    {
        ...
        {
            var x = i;
            var cs = new FakeTestCase(i, observer);
            testCases.Add(cs);
        }
        ...
        Assert.IsTrue(observer.MaxObservedThreadCount <= maxConcurrency,
            $"Independent Concurrency was {observer.MaxObservedThreadCount}, expected <= {maxConcurrency}");
    }
}